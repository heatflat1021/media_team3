...
commands = [
    ...
    Command("magic2", "岩を動かすイメージをしてください。", MESUREMENT_TIMES)
]
...
print("計測秒数と計測回数を指定しますか？指定する場合はMを、指定しない場合はそれ以外を入力してください。")
print("入力:", end="")
s = input()
if(s in "mM"):
    record_seconds = record_num = 0
    while True:
        print("計測秒数:", end="")
        n = input()
        try:
            record_seconds = int(n)
        except:
            continue
        break
    ...
    MESUREMENT_SECONDS = record_seconds
    for command in commands:
        command.mesurement_times = record_num
...
for idx, command in enumerate(commands):
    ...
    print("{}秒間の計測を{}セット行います。".format(MESUREMENT_SECONDS, command.mesurement_times))
